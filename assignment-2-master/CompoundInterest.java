...
public class CompoundInterest  extends Application{
	public void start(Stage status) {
		...
		cal.setOnAction(e -> {
			counter.setText("In total that will be " + (Math.round(Double.valueOf(startAmount.getText())* Math.pow((1 + (Double.valueOf(interest.getText()) / 100)), Double.valueOf(year.getText())))));
		});
